...
import { DataGridDefaultRenderersProvider, useDefaultRenderers } from "./DataGridDefaultRenderersProvider";
...
import { focusSinkClassname, focusSinkHeaderAndSummaryClassname, rootClassname, viewportDraggingClassname } from "./style/core";
...
type DefaultColumnOptions<R, SR> = Pick<Column<R, SR>, "renderCell" | "width" | "minWidth" | "maxWidth" | "resizable" | "sortable" | "draggable">;
...
type SharedDivProps = Pick<
  ...
  "role" | "aria-label" | "aria-labelledby" | "aria-describedby" | "aria-rowcount" | "className" | "style"
>;
 ...
 */
function DataGrid<R, SR, K extends Key>(props: DataGridProps<R, SR, K>, ref: React.Ref<DataGridHandle>) {
  const {
    // Grid and data Props
    columns: rawColumns,
    rows,
  ...
  const [resizedColumnWidths, setResizedColumnWidths] = useState((): ReadonlyMap<string, number> => new Map());
  const [measuredColumnWidths, setMeasuredColumnWidths] = useState((): ReadonlyMap<string, number> => new Map());
  ...
  function getRowViewportColumns(rowIdx: number) {
    ...
    if (selectedColumn !== undefined && selectedPosition.rowIdx === rowIdx && !viewportColumns.includes(selectedColumn)) {
      ...
      return selectedPosition.idx > colOverscanEndIdx
        ...
        : [...viewportColumns.slice(0, lastFrozenColumnIndex + 1), selectedColumn, ...viewportColumns.slice(lastFrozenColumnIndex + 1)];
    }
  ...
  return (
      ...
      style={
        {
          ...
          scrollPaddingInlineStart:
            selectedPosition.idx > lastFrozenColumnIndex || scrollToPosition?.idx !== undefined ? `${totalFrozenColumnWidth}px` : undefined,
          scrollPaddingBlock:
            isRowIdxWithinViewportBounds(selectedPosition.rowIdx) || scrollToPosition?.rowIdx !== undefined
              ? `${headerRowsHeight + topSummaryRowsCount * summaryRowHeight}px ${bottomSummaryRowsCount * summaryRowHeight}px`
              : undefined,
          gridTemplateColumns,
    ...
    >
      ...
      {scrollToPosition !== null && (
        <ScrollToCell scrollToPosition={scrollToPosition} setScrollToCellPosition={setScrollToPosition} gridElement={gridRef.current!} />
      )}
    </div>
  );
}
